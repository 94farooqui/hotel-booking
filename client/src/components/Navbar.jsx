      ...
      <nav className="w-[1000px] max-w-[1000px] mx-auto">
        <div className="container w-full flex justify-between items-center ">
          <Link to="/" className="text-2xl font-bold">
            <span className="text-gray-900"><span className="text-indigo-500">Hoteling</span>.com</span>
          </Link>
          <div className="space-x-8 flex py-4 text-gray-700">
            ...
            <button onClick={()=>setShowCurrencyCountryModal(true)} className="flex items-center gap-2"><FaGlobe className="text-xl"/>{language}.{currency}</button>
            {user ? (
              <>
            ...
            <button onClick={()=>setShowUserMenu(true)} className="flex  items-center gap-2"><IoMenu className="text-xl"/> Menu</button>
            {showUserMenu && <div ref={menuRef} className="absolute top-8 z-30 bg-white rounded-md border">
              <ul className="w-24">
        ...
        <div className="absolute top-0 left-0 z-20 bg-gray-800 bg-opacity-70 w-screen h-screen flex justify-center items-center">
          <div className="w-[400px] p-6 bg-white rounded-lg">
            <div className="flex justify-between items-center mb-4">
              ...
              <button className="font-semibold" onClick={()=>setShowCurrencyCountryModal(false)}><IoClose/></button>
            </div>
